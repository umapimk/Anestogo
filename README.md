...
- Adds **Clinical Approach** as a separate top-level module; Crisis remains independent.
- Provides 12 symptom-first entry presentations.
- Includes local semantic/relationship search.
- Implements the first interactive pathway for **Hypotension**.
- Links suspected diagnoses from Clinical Approach into the related Crisis Protocol.
- Uses a new service-worker cache (`v0701-r1`) and cache-busted assets to replace v0.63.3 reliably.

> Clinical decision-support prototype. Verify all clinical content against current approved institutional protocols.

---

# Anesthculator v0.63.3
...
## What changed in v0.63.3

v0.63.3 is a **safety and correctness release**. No new clinical content was
...
| Record | Dose | Stock | v0.60 showed | v0.63.3 shows |
|---|---|---|---|---|
...
## v0.63.3 safety hardening status
...
- Service-worker cache is versioned `v0633-r1` and uses cache-first/stale-while-revalidate for the same-origin app shell.
...
## v0.63.3 — Interactive Crisis Checklist
...
## v0.63.3 — Sticky clock and guided reassessment

- The active crisis elapsed-time display is now a compact fixed bar at the bottom of the viewport and remains visible while the checklist scrolls.
- Reassessment nodes are actionable buttons rather than static text.
- Choosing a reassessment result records the branch in the timeline, completes the associated action step, and scrolls to the next algorithm box defined by that branch.
- The chosen branch is visually retained and restored with the active crisis state after reload.
